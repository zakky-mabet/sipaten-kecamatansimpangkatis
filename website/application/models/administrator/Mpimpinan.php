...
class Mpimpinan extends CI_Model 
{
	...
	$this->db->insert('tb_camat', $data);


	if($this->db->affected_rows()){

		$this->template->alert(
			' Data Pimpinan ditambahkan.', 
			array('type' => 'success',
				'icon' => 'check')
			);
	} 
	...
	public function get($param = 0)
	{
		...
		return $this->db->get('tb_camat')->row();
	}
	...
	$data = array(
		...
		$this->db->update('tb_camat', $data, array('id_camat' => $param));

		if($this->db->affected_rows())
		{
			$this->template->alert(
				' Data Pimpinan berhasil diubah.', 
	...
	public function delete($param = 0)
	{
		...
		$this->db->delete('tb_camat', array('id_camat' => $param));

		if($this->db->affected_rows())
		{
			$this->template->alert(
				' Data Pimpinan berhasil dihapus.', 
				array('type' => 'success','icon' => 'check')
			);
		} else {
			$this->template->alert(
	...
	public function multiple_delete()
	{
		if( is_array($this->input->post('data')) )
		{
			foreach ($this->input->post('data') as $key => $value) 
				...
				@unlink("assets/images/team/{$this->get($value)->foto}");

				$this->db->delete('tb_camat', array('id_camat' => $value));
