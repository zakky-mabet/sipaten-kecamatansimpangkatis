			...
			<div class="box-body">
				...
				<div class="form-group">
				  <label>Tampilkan :</label><br>
					...
					<select name="per_page" class="form-control input-sm" style="width:60px; display: inline-block;" onchange="window.location = '<?php echo site_url('administrator/galeri?per_page='); ?>' + this.value + '&query=<?php echo $this->input->get('query'); ?>&status=<?php echo $this->input->get('status'); ?>&user=<?php echo $this->input->get('user'); ?>';">
					<?php  
					/**
					 * Loop 10 to 100
				...
				<div class="col-md-3">
				    <div class="form-group">
				        <label>Album :</label>
				        <select name="album" class="form-control input-sm">
				        	<option value="">-- PILIH --</option>
				        	<?php foreach ($this->setting->album() as $key => $value): ?>
				        		<option value="<?php echo $value->id ?>" <?php if($this->input->get('album')==$value->id) echo 'selected'; ?>><?php echo $value->title;  ?></option>
				        	<?php endforeach ?>
...
?>
			...
			<?php	}
				...
				foreach($data as $row) :
					...
					foreach ($this->setting->album() as $key => $value) {
						if ($row->id_album_galeri == $value->id) {
							$albumtext = $value->title;
							$hits =  $value->hits;
						}
						}
				...
				?>
						<tr>
							...
							<td class="text-center"><?php echo $albumtext; ?></td>
							...
							<a class="icon-button text-red get-delete-galeri" data-id="<?php echo $row->id; ?>" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash-o"></i></a>
							<?php } ?>
							</td>
						</tr>
				<?php  
    ...
    * Modal Delete 
    */
    $('.get-delete-galeri').click( function() 
    {
        $('#modal-delete-galeri').modal('show');

        $('a#btn-delete').attr('href', base_url + '/administrator/galeri/delete/' + $(this).data('id'));
    });
				</script>
					</tbody>
					<tfoot>
...
<div class="modal animated fadeIn modal-danger" id="modal-delete-galeri" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
           	<div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
