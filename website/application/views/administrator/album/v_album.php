...
?>			
			<div class="box-body">
				...
				<div class="form-group">
				  <label>Tampilkan :</label><br>
					...
					<select name="per_page" class="form-control input-sm" style="width:60px; display: inline-block;" onchange="window.location = '<?php echo site_url('administrator/album?per_page='); ?>' + this.value + '&query=<?php echo $this->input->get('query'); ?>&status=<?php echo $this->input->get('status'); ?>&user=<?php echo $this->input->get('user'); ?>';">
					<?php  
					/**
					 * Loop 10 to 100
				...
				<div class="col-md-3 top">
					  <a href="<?php echo site_url('administrator/album/create') ?>" class="btn btn-warning hvr-shadow" style="margin-left: 10px;"><i class="fa fa-plus"></i> Tambah Baru</a>
				</div>
			</div>
			...
			<div class="box-body">
				...
				<div class="col-md-3">
                    ...
                    <a href="<?php echo site_url('administrator/album') ?>" class="btn btn-warning hvr-shadow top" style="margin-left: 10px;"><i class="fa fa-times"></i> Reset</a>
				    </div>
				</div>
 ...
 * @return string
 **/
echo form_open(site_url('administrator/album/bulk_action'));
?>
			...
			<?php	}
				...
				?>
						<tr>
							...
							<a href="<?php echo base_url("administrator/album/status/{$row->id}"); ?>" class="icon-button text-orange" data-toggle="tooltip" data-placement="top" title="Status"><i class="fa <?php echo eye_status($row->status); ?>"></i></a>

							<a href="<?php echo base_url("administrator/album/get/{$row->id}"); ?>" class="icon-button text-blue" data-toggle="tooltip" data-placement="top" title="Sunting"><i class="fa fa-pencil"></i></a>

							<?php if ($this->session->userdata('akun_hak_admin') != 'operator') { ?>
							<a class="icon-button text-red get-delete-album" data-id="<?php echo $row->id; ?>" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash-o"></i></a>
							<?php } ?>
							</td>
						</tr>
				<?php  
    ...
    * Modal Delete 
    */
    $('.get-delete-album').click( function() 
    {
        $('#modal-delete-album').modal('show');

        $('a#btn-delete').attr('href', base_url + '/administrator/album/delete/' + $(this).data('id'));
    });
				</script>
					</tbody>
					<tfoot>
...
<div class="modal animated fadeIn modal-danger" id="modal-delete-album" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
           	<div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
