...
?>			
			<div class="box-body">
				<div class="col-md-3">
					Tampilkan 
					<select name="per_page" class="form-control input-sm" style="width:60px; display: inline-block;" onchange="window.location = '<?php echo site_url('administrator/pimpinan?per_page='); ?>' + this.value + '&query=<?php echo $this->input->get('query'); ?>&status=<?php echo $this->input->get('status'); ?>&user=<?php echo $this->input->get('user'); ?>';">
					<?php  
					/**
					 * Loop 10 to 100
				...
				<div class="col-md-3">
					  <a href="<?php echo site_url('administrator/pimpinan/create') ?>" class="btn btn-warning hvr-shadow" style="margin-left: 10px;"><i class="fa fa-plus"></i> Tambah Baru</a>
				</div>
			</div>
			...
			<div class="box-body">
				...
				<div class="col-md-3">
                    ...
                    <a href="<?php echo site_url('administrator/pimpinan') ?>" class="btn btn-warning hvr-shadow top" style="margin-left: 10px;"><i class="fa fa-times"></i> Reset</a>
				    </div>
				</div>
 ...
 * @return string
 **/
echo form_open(site_url('administrator/pimpinan/bulk_action'));
?>
				...
				?>
						<tr>
							<td>
			                    ...
			                    <div class="checkbox checkbox-inline">
			                        <input id="checkbox1" type="checkbox" name="data[]" value="<?php echo $row->id_camat; ?>"> <label for="checkbox1"></label>
			                    </div>
						
							</td>
							<td><?php echo $row->nama_lengkap; ?></td>
							<td class="text-center"><?php echo $row->nip; ?></td>
							<td class="text-center"><?php echo $row->tmp_lahir.', '. date_id($row->tgl_lahir) ?></td>
							...
							<a href="<?php echo base_url("administrator/pimpinan/status/{$row->id_camat}"); ?>" class="icon-button text-orange" data-toggle="tooltip" data-placement="top" title="Status"><i class="fa <?php echo eye_status($row->status); ?>"></i></a>

							<a href="<?php echo base_url("administrator/pimpinan/get/{$row->id_camat}"); ?>" class="icon-button text-blue" data-toggle="tooltip" data-placement="top" title="Sunting"><i class="fa fa-pencil"></i></a>

							<?php if ($this->session->userdata('akun_hak_admin') != 'operator') { ?>
							<a class="icon-button text-red get-delete-pimpinan" data-id="<?php echo $row->id_camat; ?>" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="fa fa-trash-o"></i></a>
							<?php } ?>
							</td>
						</tr>
				<?php  
    ...
    * Modal Delete 
    */
    $('.get-delete-pimpinan').click( function() 
    {
        $('#modal-delete-pimpinan').modal('show');

        $('a#btn-delete').attr('href', base_url + '/administrator/pimpinan/delete/' + $(this).data('id'));
    });
				</script>
					</tbody>
					<tfoot>
...
<div class="modal animated fadeIn modal-danger" id="modal-delete-pimpinan" data-backdrop="static" data-keyboard="false">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
           	<div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
