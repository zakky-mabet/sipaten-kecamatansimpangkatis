...
// inspired from the source >> https://github.com/nobuti/Codeigniter-adminbreadcrumbs
class Adminbreadcrumbs {

    protected $CI;

    private $adminbreadcrumbs = array();
    ...
    public function push($id, $page, $url)
    {
		...
		$this->adminbreadcrumbs[$url] = array('id' => $id, 'page' => $page, 'href' => $url);
	}
	...
	public function unshift($id, $page, $url)
	{
		...
		array_unshift($this->adminbreadcrumbs, array('id' => $id, 'page' => $page, 'href' => $url));
	}


	public function show()
	{
		if ($this->adminbreadcrumbs)
        {
			$output = $this->_open ."\n";

            usort($this->adminbreadcrumbs, $this->array_sorter('id'));

			foreach ($this->adminbreadcrumbs as $key => $value)
            {
				$keys = array_keys($this->adminbreadcrumbs);
