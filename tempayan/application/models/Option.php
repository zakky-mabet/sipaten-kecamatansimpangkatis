...
class Option extends Sipaten_model 
{
	private $data = array();
	...
	{
		return $this->db->query("SELECT users.firebase_token FROM pegawai RIGHT JOIN users ON users.nip = pegawai.nip WHERE pegawai.ID = ?", array($param))->row('firebase_token');
	}
}
